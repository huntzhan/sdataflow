...
Note that the name of outcome type is the string embraced in brackets(**not** including the brackets).

## Register Callback

`sdataflow` provides a class `DataflowHandler` to parse `doc`(a string represents the relations of entries), register callbacks and schedule the execution of callbacks.
